...
export default function WeatherMap({ forecast }: WeatherMapProps) {
  ...
  // Memoize values used inside initMap so the effect dependency is accurate
  const village = forecast.region.village;
  const district = forecast.region.district;
  const city = forecast.region.city;
  const pt = useMemo(
    () => forecast.nearestPoint ?? forecast.days[0]?.points[0],
    [forecast.nearestPoint, forecast.days]
  );
  ...
  }, [lat, lon, village, district, city, temp]);
    ...
    <div className="weather-card rounded-3xl p-card-padding sky-shadow flex flex-col">
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-xs text-text-muted">
        <span className="bg-primary/10 px-2.5 py-1 rounded-full font-geist flex items-center gap-1">
          <span className="material-symbols-outlined text-[14px]">location_on</span> {village}, {city}
        </span>
        <span className="bg-primary/10 px-2.5 py-1 rounded-full font-geist flex items-center gap-1">
          <span className="material-symbols-outlined text-[14px]">thermostat</span> {pt?.temperatureC ?? "--"}°C
        </span>
        <span className="bg-primary/10 px-2.5 py-1 rounded-full font-geist flex items-center gap-1">
          <span className="material-symbols-outlined text-[14px]">air</span> {pt?.windSpeedKmh ?? "--"} km/j
        </span>
        <span className="bg-primary/10 px-2.5 py-1 rounded-full font-geist flex items-center gap-1">
