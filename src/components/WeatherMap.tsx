  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      try {
        ...
        L.circle([lat, lon], { radius: 5000, color, fillColor: color, fillOpacity: 0.15, weight: 2, opacity: 0.6 }).addTo(map);
    ...
    if (mapInstance.current) { mapInstance.current.remove(); mapInstance.current = null; setMapReady(false); }
    initMap();
